...
const PairSelector = ({ formik, disabled }) => {
  ...
  return (
    <TymioUI.Select custom noMargin ref={ref} width={140} show={show}>
      <TymioUI.Select.Value custom onClick={handleClick}>
        <TymioUI.TokenIcon token={formik.values.tokenSymbol} />
      ...
      <TymioUI.Select.Options custom show={show} side={'right'}>
          ...
          options.map((token, index) => {
            if (token.tokenAddress !== formik.values.tokenAddress) {
              return (
                ...
                >
                  <TymioUI.TokenIcon token={token.tokenSymbol} />
